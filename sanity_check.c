...
static int wayland_display_connect() {
  ...
  if (xdg_runtime_dir == NULL) return EINVAL;
  ...
  if (connect(fd, (struct sockaddr *)&addr, sizeof(addr)) == -1) exit(errno);
...
static uint32_t wayland_wl_display_get_registry(int fd) {
  ...
  if ((int64_t)msg_size != send(fd, msg, msg_size, 0)) exit(errno);
...
static uint32_t wayland_wl_registry_bind(int fd, uint32_t registry, uint32_t name, char *interface,
                                         uint32_t interface_len, uint32_t version) {
  uint64_t msg_size = 0;
  char msg[512] = "";
...
static void create_shared_memory_file(uint64_t size, state_t *state) {
  ...
  if (ftruncate(fd, size) == -1) exit(errno);
...
static uint32_t wayland_wl_shm_create_pool(int fd, state_t *state) {
  ...
  *((int *)CMSG_DATA(cmsg)) = state->shm_fd;
  ...
  if (sendmsg(fd, &socket_msg, 0) == -1) exit(errno);
...
static uint32_t wayland_xdg_wm_base_get_xdg_surface(int fd, state_t *state) {
  ...
  printf("-> xdg_wm_base@%u.get_xdg_surface: xdg_surface=%u wl_surface=%u\n", state->xdg_wm_base,
         wayland_current_id, state->wl_surface);
...
static void wayland_handle_message(int fd, state_t *state, char **msg, uint64_t *msg_len) {
  ...
  if (object_id == state->wl_registry && opcode == wayland_wl_registry_event_global) {
    ...
    assert(announced_size == sizeof(object_id) + sizeof(announced_size) + sizeof(opcode) + sizeof(name) +
    ...
    if (strcmp(wl_shm_interface, interface) == 0) {
      state->wl_shm =
        wayland_wl_registry_bind(fd, state->wl_registry, name, interface, interface_len, version);
    }
...
int main() {
  ...
  while (1) {
    ...
    if (read_bytes == -1) exit(errno);

    char *msg = read_buf;
    uint64_t msg_len = (uint64_t)read_bytes;

    while (msg_len > 0) wayland_handle_message(fd, &state, &msg, &msg_len);
    ...
    if (state.state == STATE_SURFACE_ACKED_CONFIGURE) {
      ...
      if (state.wl_shm_pool == 0) state.wl_shm_pool = wayland_wl_shm_create_pool(fd, &state);
      if (state.wl_buffer == 0) state.wl_buffer = wayland_wl_shm_pool_create_buffer(fd, &state);
