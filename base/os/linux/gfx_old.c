static u32 WaylandRegistryBind(u32 Fd, u32 CurrentId, u32 Registry, u32 Name, u32 Version, u32 InterfaceLen,
    ...
    *(u32 *)Buff = CurrentId;
    ...
    u64 Sent = send(Fd, Temp, Header.Size, MSG_DONTWAIT);
...
static void WaylandInit(wayland_context *WlCtx, u32 Height, u32 Width, string8 Title) {
    ...
    *(u32 *)Buff = CurrentId;
    ...
    Buff = Temp;
    Sent = send(Fd, Buff, Header.Size, MSG_DONTWAIT);
    Assert(Header.Size == Sent);
    ...
    *(wayland_header *)Buff = Header;
    ...
    #undef X
            ...
            *(u32 *)(Buff + sizeof(wayland_header)) = Configure;
            ...
            Sent = send(Fd, Buff, SendHeader.Size, MSG_DONTWAIT);
            Assert(Header.Size == Sent);
...
static void WaylandCommitSurface(wayland_context *WlCtx) {
    ...
    *(wayland_header *)Buff = Header;
    u64 Sent                = send(WlCtx->Fd, Buff, Header.Size, MSG_DONTWAIT);
    Assert(Header.Size == Sent);
    (void)Sent;
        ...
        while ((RingBuffer.Written > RingBuffer.Read) && (State != WLSTATE_Error)) {
            ...
            if (Header.ObjectId == XdgWmBase && Header.OpCode == XDG_WM_BASE_EVENT_PING) {
                ...
                *(u32 *)(Buff + sizeof(wayland_header)) = Ping;
                ...
                u64 Sent = send(Fd, Buff, SendHeader.Size, MSG_DONTWAIT);
                Assert(SendHeader.Size == Sent);
                (void)Sent;
